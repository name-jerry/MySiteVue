//抛物线运动的物体,创建时初始本体坐标,初速度,目标坐标
// 自带方法,updateAim根据目标点自动更新自身数据,
// z轴作为高度
const PaoWuZ = class {
  // 初始属性
  createXYAngle; //xy面角度
  ...
  constructor(createX, createY, v, aimX, aimY) {
    //创建初始属性,z为高度目前只支持初始高度0
    this.createX = createX;
    this.createY = createY;
    this.aimX = aimX || createX;
    this.aimY = aimY || createY;
    this.createSV = v || this.getMinVByMaxXY(innerWidth / 2, innerHeight / 2);
    this.updateAim();
    // 瞬时属性
  ...
  updateAim(aimX = this.aimX, aimY = this.aimY) {
    ...
    this.maxS = Math.pow(this.createSV, 2) / this.g;
    //实际距离
  ...
  getCurrentSite(intervalTime) {
    ...
    this.currentZ =
      Math.sin(this.createZAngle) * this.createSV * time -
